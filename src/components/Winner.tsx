...
export default function Winner({ project, position }: WinnerProps) {
    ...
    const imagesSorted = project.images.sort((a, b) => a.aiTags.includes("title-image") ? -1 : 0)
    return (
        <div className="flex flex-col items-center justify-center mb-10 last:mb-0 text-center">
            ...
            <Link className="font-bold underline text-2xl mb-4 hover:opacity-75" href={architectUrl}>{architectName}</Link>
            <h4 className="text-xl mb-4">{project.title}</h4>
            <p className="text-m text-center px-10 max-w-[800px]">{project.description}</p>
